...
'@
$insert = @'
INSERT INTO [Powershell].[dbo].[EndpointStatus2]
(Timestamp, Endpoint_ID, Status, ResponseTime)
VALUES ('{0}','{1}','{2}','{3}')
'@
...
while ($true) {
	Try {
		...
		for ($i = 0; $i -lt $DataSet.Tables.Rows.Length; $i++) {
			...
			$cmd.CommandText = $insert -f $Timestamp, $DataSet.Tables.Endpoint_ID[$i], $Status, $ResponseTime
			$cmd.ExecuteNonQuery()
